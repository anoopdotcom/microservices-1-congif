...
import com.CropDeal.AuthService.service.implementation.UserServiceImplementation;
    ...
    public Mono<ResponseEntity<String>> login(String email, String password) {
        ...
        return userOptional.map(user -> {
            if (user.getUsername().equals(email) && passwordEncoder.matches(password, user.getPassword())) {
                return Mono.just(ResponseEntity.ok(jwtService.generate(user.getUsername())));
            } else {
